...
#define STATS_LOCK   LOCK_STATS
#define STATS_UNLOCK UNLOCK_STATS

void *ssl_accept(conn *c, int sfd, bool *fail) {
    SSL *ssl = NULL;
    if (c->ssl_enabled) {
        assert(!IS_UDP(c->transport) && settings.ssl_enabled);
        ...
        if (ret <= 0) {
            ...
            if (err == SSL_ERROR_WANT_WRITE || err == SSL_ERROR_WANT_READ) {
                ...
                mc_logger->log(EXTENSION_LOG_WARNING, c, "type=conntlserr fd=%d errno=%d msg=%s\n",
...
static bool load_server_certificates(char **errmsg) {
    ...
    SSL_LOCK();
    if (!SSL_CTX_use_certificate_file(settings.ssl_ctx,
        settings.ssl_chain_cert, settings.ssl_keyformat)) {
